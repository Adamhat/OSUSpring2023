...
main( int argc, char *argv[ ] )
{
	...
	cl_mem mW = clCreateBuffer( Context, CL_MEM_READ_ONLY,  mwSize, NULL, &status );
	if( status != CL_SUCCESS )
	...
	status = clEnqueueWriteBuffer( CmdQueue, mW, CL_FALSE, 0, mwSize, &mw, 0, NULL, NULL );
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg( Kernel, 2, sizeof(cl_mem), &mW );
	if( status != CL_SUCCESS )
