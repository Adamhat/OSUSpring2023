...
def mymerge(k_lists):
    ...
    while heap != []:
        ...
        yield x
        a = k_lists[i]
        if j < len(a) - 1: # this list is not empty
            heappush(heap, (a[j+1], i, j+1))

def mymerge1(k_lists): # heapreplace instead of heappop
    heap = [(a[0], i, 0) for i, a in enumerate(k_lists)]
    heapify(heap)
    while heap != []:
        x, i, j = heap[0] # peek, not pop 
        yield x
        a = k_lists[i]
        if j < len(a) - 1: # this list is not empty
            heapreplace(heap, (a[j+1], i, j+1))
        else: # this list is empty; just pop
            heappop(heap)
...
def kmergesort(m, k=2):
    ...
    return list(mymerge1(k_lists))
    #return list(mymerge(k_lists))
    #return list(merge(*k_lists)) # using builtin heapq.merge
